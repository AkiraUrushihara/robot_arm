        ...
        try:
            ...
            if n == "gripper":
                ...
                self.get_logger().info(
                    f"[gripper debug] home={home}, low={low}, high={high}, rad={rad:.3f}, raw(before clamp)={int(home + rad * self._steps_per_rad)}"
                )

            # 書き込み前に追加
            self.get_logger().info(f"Planned raw goals (gripper): {raw_goals.get('gripper')}")
