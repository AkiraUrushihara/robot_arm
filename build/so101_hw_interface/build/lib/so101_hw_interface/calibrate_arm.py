...
class Calibrator(Node):
    ...
    def run(self):
        ...
        for joint in JOINTS:
            ...
            neg_deg = neg * 360.0 / 4096.0
            print(f"  NEGATIVE position raw={neg}  ({neg_deg:.2f}°)")
            ...
            print(f"  POSITIVE position raw={pos}  ({pos_deg:.2f}°)")

            # ensure correct ordering
            range_min = int(min(neg, pos))
            range_max = int(max(neg, pos))
            home = int((range_min + range_max) // 2)
            home_deg = home * 360.0 / 4096.0

            print(f"  → Computed home = {home}  ({home_deg:.2f}°)\n")
